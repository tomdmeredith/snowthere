            ...
            <p className="mt-8 text-xl text-dark-600 leading-relaxed max-w-2xl">
              {filteredResorts.length > 0 ? (
                selectedAges.length > 0 ? (
                  <>
                    <strong className="text-coral-500">{filteredResorts.length}</strong> resorts perfect for {ageDisplayText} with complete trip guides, cost breakdowns, and detailed family information.
                  </>
                ) : (
                  <>
                    <strong className="text-coral-500">{filteredResorts.length}</strong> family-friendly resorts worldwide with complete trip guides, cost breakdowns, and detailed family information.
                  </>
                )
              ) : (
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {selectedAges.map((age) => {
                ...
                const href = remainingAges.length > 0
                  ? `/resorts?ages=${remainingAges.join(',')}`
                  : '/resorts'
